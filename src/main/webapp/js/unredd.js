...
$(window).load(function () {
            ...
            jQuery.each(context.layers, function (n, layer) {
                ...
                if (layerConf.visible && typeof layerConf.legend !== "undefined") {
                    ...
                    if (!contextIsActive) {
                        ...
                        table += '<td colspan="2" style="width:100%;background-color:white"><img src="http://zmb-nfms.org/stg_geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&LAYER=' + layerConf.wmsName + '" /></td>';
                        table += '</tr>';
                        table += '</table>';
                    }
            ...
            jQuery.each(contextGroups.items, function (contextGroupName, contextGroupDefinition) {
                ...
                if (contextGroupDefinition.hasOwnProperty('group')) {
                    ...
                    if (level === 0) {
                        ...
                        innerElement = $('<table style="width:100%;border-collapse:collapse"><tr></tr></table>');
		...
		var flag = true;
			...
			if(flag){
				...
				return true;			
                    ...
                    td1.append(info);//info.title().toLowerCase());
    ...
    */
    ...
    printLayer = new OpenLayers.Layer.Vector("Print");  
    UNREDD.map.addLayer(printLayer);

    // Print button
    $("#button_print").bind('click', function () {
	if (!$("#button_print").hasClass('selected') && !$("#button_print").hasClass('selected')) {
		activatePrintDialog();
	}
	return false;
    });
